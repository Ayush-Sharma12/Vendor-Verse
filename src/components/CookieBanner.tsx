...
// Generic tracking - loads script and tracks clicks/pageviews
function initTracking(): void {
  ...
  const track = (eid: string, type: string, label: string, props?: Record<string, unknown>) => {
    window._signalsDataLayer!.push({
      ...
      data: { eid, type, event_label: label, custom_properties: { ...props, timestamp: new Date().toISOString(), source: 'app' } }
    });
  };
  ...
  // Initial events
  track('app.website.session', 'session', 'start', { page_path: location.pathname, referrer: document.referrer });
  track('app.website.pageview', 'pageview', document.title, { page_path: location.pathname, referrer: document.referrer });
  ...
  document.addEventListener('click', (e) => {
    ...
    track('app.website.click', 'click', text || type, {
      element_type: type,
      element_text: text,
  ...
  const trackPage = () => {
    if (location.href !== lastUrl) {
      track('app.website.pageview', 'pageview', document.title, { page_path: location.pathname, referrer: lastUrl });
  ...
  script.src = 'https://img1.wsimg.com/signals/js/clients/scc-c2/scc-c2.min.js';
  script.async = true;
 ...
 *
 * Displays a consent banner for analytics tracking. Manages user consent
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <p id="cookie-banner-description" className="text-sm text-gray-600">
              ...
              <a href="#privacy-policy" className="text-blue-600 hover:text-blue-800 underline" target="_blank" rel="noopener noreferrer">Learn more</a>
            </p>
          </div>
